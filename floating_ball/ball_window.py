...
from PyQt6.QtCore import Qt, QTimer, QSettings, QRect, QElapsedTimer, QThread, pyqtSignal
...
from themes.opus_audio import OpusAudio, get_available_tracks
...
class APIWorker(QThread):
    ...
    def run(self):
        try:
            ...
            self.error.emit(e.error_type, e.message)
...
class BallWindow(QWidget):
    ...
    def refresh_data(self):
        ...
        self.api_worker = APIWorker(self.api_service)
        self.api_worker.finished.connect(self.on_refresh_finished)
        self.api_worker.error.connect(self.on_refresh_error)
        self.api_worker.start()

    def on_refresh_finished(self, data):
        """Handle successful API refresh."""
    ...
    def on_refresh_error(self, error_type, message):
        """Handle API refresh error."""
        self.error_message = message
        print(f"API error: {message}")

        if error_type in ("session_expired", "forbidden", "not_authenticated"):
            self.needs_login = True
            QTimer.singleShot(100, self.prompt_relogin)
        elif error_type == "network_error" or ("API error:" in message and "50" in message):
            self.error_message = "Claude is down"
        elif error_type == "rate_limit":
            self.error_message = "Rate limited"
    ...
    def create_context_menu(self):
        ...
        # Music submenu (opus theme only)
        self.music_menu = menu.addMenu("Music")
        self.music_actions = []
        self._build_music_menu()
        self._update_music_visibility()
    ...
    def _build_music_menu(self):
        """Populate the music submenu with available tracks."""
        self.music_menu.clear()
        self.music_actions = []

        # "Off" option
        off_action = self.music_menu.addAction("Off")
        off_action.setCheckable(True)
        off_action.setChecked(True)
        off_action.triggered.connect(lambda: self._select_music(-1))
        self.music_actions.append(off_action)

        self.music_menu.addSeparator()

        # Track options
        for idx, (name, track_type, path) in enumerate(get_available_tracks()):
            action = self.music_menu.addAction(name)
            action.setCheckable(True)
            action.setChecked(False)
            action.triggered.connect(lambda checked, i=idx: self._select_music(i))
            self.music_actions.append(action)

    def _select_music(self, track_index):
        """Select a music track or turn off. -1 = off, 0+ = track index."""
        if track_index < 0:
            self.opus_audio.stop()
        else:
            self.opus_audio.start(track_index)

        # Update checkmarks (radio-button style)
        for i, action in enumerate(self.music_actions):
            if i == 0:
                action.setChecked(track_index < 0)
            else:
                action.setChecked(i - 1 == track_index)

    def _update_music_visibility(self):
        """Show music menu only when opus theme is active."""
        theme_name = self.settings.value("theme", "space", type=str)
        is_opus = theme_name == "opus"
        self.music_menu.menuAction().setVisible(is_opus)
        if not is_opus and self.opus_audio.is_playing():
            self.opus_audio.stop()
            self._select_music(-1)
    ...
    def reload_with_theme(self, theme_name: str, color_theme_name: str):
        ...
        # Update music toggle visibility (stops music if leaving opus)
        self._update_music_visibility()
    ...
    def paintEvent(self, event):
        ...
        if self.error_message or self.needs_login:
            ...
            if self.needs_login:
                error_label = "login"
            elif "down" in getattr(self, "error_message", "").lower():
                error_label = "down"
            elif "limit" in getattr(self, "error_message", "").lower():
                error_label = "limit"
            else:
                error_label = "error"
